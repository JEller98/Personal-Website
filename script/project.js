...
.catch(error => {
    ...
    //notify the user that project data failed to load
    const grid = document.querySelector(".project-grid");
    if (grid) {
        grid.innerHTML = "<p>Projects failed to load. Please try refreshing the page.</p>";
    }
});
...
window.addEventListener("resize", () => {
    ...
    resizeTimer = setTimeout(updateProjectVisibility, 150);
});
...
function openModal() {
    ...
    //prevent screen readers from targeting base page elements when modal window is active
    bgRegions.forEach(el => el.inert = true);
...
function closeModal() {
    ...
    //window is closing, restore screen reader targetability
    bgRegions.forEach(el => el.inert = false);
...
function trapFocus() {
    focusTrapController = new AbortController();

    const focusArray = modal.querySelectorAll("a[href], button, [tabindex='-1']");
    ...
    }, { signal: focusTrapController.signal });
